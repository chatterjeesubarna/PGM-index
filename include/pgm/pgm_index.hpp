...
protected:
    ...
    auto segment_for_key(const K &key) const {
        ...
        // std::cout << "level access: " << count_level << std::endl;
        return it;
    }
...
public:
     ...
     */
    size_t size_in_bytes() const { std::cout << "PGM nodes: " << levels_offsets.size() << std::endl; return segments.size() * sizeof(Segment) + levels_offsets.size() * sizeof(size_t); }
};
